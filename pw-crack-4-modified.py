...
def level_4_pw_check(user_pw): #we add in user_pw as a parameter, so we can call the function with our guess, simplying things
    #user_pw = input("Please enter correct password for flag: "), we comment this out so we can use the function in our way
...
#level_4_pw_check(), we comment this out to avoid the normal check
...
for potential_password in pos_pw_list: #loops through potential passwords
    level_4_pw_check(potential_password)
